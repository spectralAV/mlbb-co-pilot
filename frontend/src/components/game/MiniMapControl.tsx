...
export function MiniMapControl({ state, onChange }: { state: GameState; onChange: (patch: Partial<GameState>) => void }) {
  ...
  return <GamePanel title="Tactical Map Control" icon={Map}>
    <div className="relative h-[420px] overflow-hidden rounded-lg border border-white/10 bg-slate-950 shadow-inner">
      ...
      {zones.map((zone) => <button key={zone.id} title="Click to cycle zone status" className={`absolute ${zone.pos} rounded-lg border px-3 py-2 text-xs font-black backdrop-blur ${statusClass[status(zone.id)]}`} onClick={() => cycle(zone.id)}>{zone.label}</button>)}
      <div className="absolute bottom-28 left-24 flex items-center text-xs font-bold text-cyan-200">
        <Route className="mr-1 h-5 w-5" /> Red to Mid to Turtle <ChevronRight className="h-5 w-5 animate-pulse" />
      </div>
      <div className="absolute bottom-4 right-4 w-52 rounded-lg border border-white/10 bg-slate-900/90 p-3">
